...
#define _WINDEF_H	//<= Temporary hack for BOOL redefinition in sybdb.h. Need to keep FreedTDS and Foundation.h separate
...
int sql_msg_handler(DBPROCESS* dbproc, DBINT msgno, int msgstate, int severity, char* msgtext, char* srvname, char* procname, int line)
{
...
int sql_err_handler(DBPROCESS* dbproc, int severity, int dberr, int oserr, char* dberrstr, char* oserrstr)
{
	//Can't call self from a C function, so need to access singleton
	SQLClient* self = [SQLClient sharedInstance];
...
void execSqlRequest() {
	...
	[client connect:@"192.168.0.32:1433" username:@"sa" password:@"Gato0474" database:@"pubs" completion:^(BOOL success) {
		if (success) {
			NSLog(@"Connected!");
		  [client execute:@"select au_lname from authors" completion:^(NSArray* results) {
